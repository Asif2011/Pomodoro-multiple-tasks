...
public class Pomodoro 
{
  ...
  static int setTomatoCount(Context context, int value) // if value < 0; count += abs(value)
  {
    ...
    String selectedTask = PomodoroActivity.getTaskSelected(context);
    int completed_session = store.getcompleted_sessions(selectedTask);
    completed_session+=1;
    store.updateTasksCompletedSession(selectedTask, ""+completed_session);
  ...
  public static class CountDownNotification extends Notification
  {
    ...
    private CountDownNotification(Context context)
    {
      ...
      mContext = context;
      mIntent = PendingIntent.getActivity(context, 0, new Intent(context, PomodoroActivity.class), 0);
    ...
    public void tick()
    {
      ...
      this.setLatestEventInfo(mContext, mContext.getString(R.string.app_name), msg, mIntent);
